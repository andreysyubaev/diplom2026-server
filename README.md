...
## 9. Перенос БД на другой компьютер (для защиты)

### Сделать дамп на твоём ноуте

В PowerShell в любой папке (файл `college_app_backup.sql` появится там же, где запустишь команду):

```powershell
$env:PGPASSWORD = 'Vlad_2018'
pg_dump -h 127.0.0.1 -U postgres -d college_app --clean --if-exists -f college_app_backup.sql
```

Что значат флаги:
- `--clean --if-exists` — в начало файла добавляются `DROP IF EXISTS`, чтобы при загрузке на другой комп старая структура сначала чистилась, а потом заливалась свежая. Безопасно прогонять повторно.
- Файл будет содержать **всё**: схему, миграции, пользователей, темы, подтемы, тесты, результаты.

Заодно скопируй папку **`uploads/`** (рядом с `server.exe` или внутри `server/`). Там лежат картинки и вложения, которые ты заливал. Без неё на новом компе вместо картинок будут битые иконки.

### Что положить на флешку

1. `college_app_backup.sql` — дамп БД
2. Папка `uploads/` — пользовательские файлы
3. `server.exe` (если компилировал) или папка с исходниками сервера
4. `.env` (или `.env.example` — на новом компе заполнишь)
5. Сборки приложения: `app-release.apk`, папка `windows/Release/`, папка `web/`

### Восстановить на демо-компе

1. **Установить PostgreSQL** (если ещё не стоит): https://www.postgresql.org/download/windows/  
   Запомни пароль для пользователя `postgres` — лучше использовать `Vlad_2018`, чтобы `.env` не править.

2. **Создать пустую базу:**
   ```powershell
   $env:PGPASSWORD = 'Vlad_2018'
   psql -h 127.0.0.1 -U postgres -c "CREATE DATABASE college_app;"
   ```

3. **Залить дамп:**
   ```powershell
   psql -h 127.0.0.1 -U postgres -d college_app -f college_app_backup.sql
   ```

4. **Скопировать `uploads/`** в папку рядом с `server.exe` (или внутрь `server/`, если запускаешь через `dart_frog dev`).

5. **Создать `.env`** рядом с `server.exe`, скопировав из `.env.example` и поставив свой `JWT_SECRET`.

6. **Запустить сервер** — двойной клик по `server.exe` (или `dart_frog dev` из папки сервера).

После этого на демо-компе всё ровно такое же, как у тебя дома: те же темы, лекции, тесты, картинки, результаты студентов.

### Если БД на новом компе нужно полностью обновить

Скрипт с `--clean --if-exists` сам всё снесёт перед заливкой, повторно прогонять безопасно. Если хочешь начать с нуля без дампа — `DROP DATABASE college_app;` и `CREATE DATABASE college_app;`, потом сервер сам прокатит миграции при первом запросе.
...
## 11. Что дальше (план второй итерации)
